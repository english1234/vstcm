...
//extern void draw_to_xyrgb(int, int, uint8_t, uint8_t, uint8_t);
...
void plot(int32_t x, int32_t y, int32_t z)
{
  ...
  const uint16_t vctr_minx = 512;
  const uint16_t vctr_miny = 512;
...
*/  
 if ((x > 4095) || (x < 1) || (y > 4095) || (y < 1))
    {
      goto_xy(2048, 2048);
      return;
    }
  
  draw_to_xyrgb(x, 4096-y, 0, (z * 15), 0);
}
