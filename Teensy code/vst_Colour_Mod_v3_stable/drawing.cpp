...
void update_goto_xy_settings() {
    x_axis_invert_mask = (v_setting[SETTINGS_MENU][6].pval == 0) ? 0x0FFF : 0x0000;
    y_axis_invert_mask = (v_setting[SETTINGS_MENU][7].pval == 0) ? 0x0FFF : 0x0000;
    dac_swap = v_setting[SETTINGS_MENU][8].pval;
    correction_enabled = (v_setting[SETTINGS_MENU][10].pval == 1);

    // Precompute correction factors as fixed-point (Q8.24)
    correction_x_factor = (int32_t)(0.000000013f * (1 << 24));
    correction_y_factor = (int32_t)(0.000000005f * (1 << 24));

	// Cache some more settings for fast access
    dwell_before = v_setting[SETTINGS_MENU][3].pval;
    dwell_after = v_setting[SETTINGS_MENU][4].pval;
    move_speed = v_setting[SETTINGS_MENU][1].pval;
