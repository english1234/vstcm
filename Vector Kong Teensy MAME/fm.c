...
static __inline__ void set_ar_ksr(FM_CH *CH,FM_SLOT *SLOT,int v,int32_t *ar_table)
{
	SLOT->KSR  = 3-(v>>6);
...
/* set decay rate */
static __inline__ void set_dr(FM_SLOT *SLOT,int v,int32_t *dr_table)
{
...
/* set sustain rate */
static __inline__ void set_sr(FM_SLOT *SLOT,int v,int32_t *dr_table)
{
...
/* set release rate */
static __inline__ void set_sl_rr(FM_SLOT *SLOT,int v,int32_t *dr_table)
{
...
static int FMInitTable( void )
{
	...
	TL_TABLE = (int32_t *)emu_Malloc(2*TL_MAX*sizeof(int));
...
//  if( SSGpris ) SSGClk( OPN->ST.index, OPN->ST.clock * 2 / SSGpris );
	...
	for( i=0 ; i < 2048 ; i++ )
	{
		...
		OPN->FN_TABLE[i] = (uint32_t)( (double)i * OPN->ST.freqbase * FREQ_RATE * (1<<7) / 2 );
	}
#if FM_LFO_SUPPORT
	/* LFO freq. table */
	{
...
extern int YM2612Init(int num, int clock, int rate, FM_TIMERHANDLER TimerHandler,FM_IRQHANDLER IRQHandler);
