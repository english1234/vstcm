...
#include "fm.h"     // needed for audio support
#include "sn76496.h"
extern "C" {
  #include "emuapi.h"
  #include "iopins.h"
...
static int readNbFiles(void) {
  int totalFiles = 0;
  File entry; 
  Serial.print("ROM path ");
  Serial.println(romspath);   
...
void * emu_Malloc(int size)
{
  ...
  if (!retval) {
    emu_printf("emu_Malloc: failed to allocate");
    emu_printf(size);
    emu_printf("fallback");
    if ( (malbufpt+size) < sizeof(malbuf) ) {
      ...
      emu_printf("emu_Malloc: failure to allocate");
    }
  }
  else {
...
#endif
#ifdef PIN_KEY_USER5 
  if ( digitalRead(PIN_KEY_USER5) == LOW ) retval |= MASK_KEY_USER5;
#endif
...
#endif  
#ifdef PIN_KEY_USER5
  pinMode(PIN_KEY_USER5, INPUT_PULLUP);
#endif 
#ifdef PIN_JOY2_BTN
...
void audio_play_sample(int16_t *bufl, int16_t *bufr, int length)
{
    int i;
    int16_t acc;
    ...
    YM2612UpdateOne(0, (int16_t **)tempBuffer, length);
    ...
    {
        int16_t psg = snd.psg.buffer[i] / 2;
